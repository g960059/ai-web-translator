...
test('shrinks a batch after the provider returns fewer translations than requested', async () => {
  ...
  try {
    ...
    await context.route('https://openrouter.ai/api/v1/chat/completions', async (route) => {
      ...
      const translations =
        fragments.length > 2
          ? fragments.slice(0, 3).map((fragment) => `${fragment} [ja]`)
          : fragments.map((fragment) => `${fragment} [ja]`);

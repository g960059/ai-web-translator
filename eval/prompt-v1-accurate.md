...
本稿では、いくつかの例外を除き、有限群のみを扱う。また、標数0の体上のベクトル空間に限定して議論を進める。標数0の代数閉体の理論は完全であるため、ある特定の標数0の代数閉体において成立する理論は、他のすべての標数0の代数閉体においても成立する。したがって、一般性を失うことなく、
...
表現論は数学の多くの分野で用いられるほか、量子化学や物理学においても利用されている。とりわけ代数学においては、群の構造を調べるために用いられる。また、調和解析や数論への応用も存在する。例えば、現代的なアプローチにおいて、保型形式に関する新たな結果を得るために表現論が用いられている。
...
本稿では、最終章を除き、有限次元表現空間の考察に限定する。多くの場合、
...
内の有限個のベクトルのみが重要であるため、それらのベクトルによって生成される部分表現を調べるだけで十分である。この部分表現の表現空間は有限次元となる。
...
の像は、1の第4冪根からなる群の非自明な部分群でなければならないという結論に至る。言い換えれば、
...
を介して同型である。このため、これらを区別せず、単に「正則表現」と呼ぶことが多い。
...
上のベクトル空間として、加法およびスカラー倍を定義すると、このベクトル空間は
...
線形に拡張する。明らかに、この写像は全単射である。上の式で示した2つの基底元の畳み込みを詳しく調べると、
...
における積に対応することがわかる。したがって、畳み込み代数と群代数は代数として同型である。
...
上のフーリエ変換を実装できる。調和解析の分野において、以下の定義は
...
の部分表現と呼ぶ。任意の表現Vは少なくとも2つの部分表現を持つ。すなわち、0のみからなるものと、V自身である。これら2つ以外に部分表現を持たない場合、その表現を既約表現と呼ぶ。一部の著者は、これらが群代数
...
上の単純加群と正確に対応することから、これらの表現を単純表現と呼ぶこともある。
...
線形ベクトル空間同型。言い換えれば、全単射な線形写像が存在する場合に同型である。
...
がユニタリである場合、ユニタリ表現と呼ばれる。これは特に、すべての
...
が対角化可能であることを意味する。詳細はユニタリ表現に関する記事を参照のこと。
...
したがって、一般性を失うことなく、今後検討するすべての表現はユニタリであると仮定できる。
...
が存在する。ゆえに、この表現は既約ではない。言及された部分表現は次数1であり、既約である。
...
例については、このトピックのメインページを参照されたい：双対表現。
...
注記。直和とテンソル積は次数が異なるため、異なる表現であることに注意されたい。
...
表現をより容易に理解するために、表現空間をより単純な部分表現の直和に分解することが望ましい。有限群については、以下の結果で示されるように、これが達成可能である。より詳細な説明と証明は [1]および [2]に見ることができる。
...
この分解は一意ではないことに注意せよ。しかしながら、与えられた既約表現と同型な部分表現がこの分解に何回現れるかという数は、分解の選択によらず一定である。

一意な分解を得るためには、互いに同型なすべての既約部分表現をまとめる必要がある。すなわち、表現空間をそのアイソタイプ（同型類）の直和に分解する。この分解は一意に決定され、標準分解と呼ばれる。
...
の非自明な既約表現とする。このとき、自明な表現へのアイソタイプは
...
はそうではないことがわかる。これは（以下の「内積と指標」で定義される内積に関して）
...
この公式は、2つの正方行列の積ABのトレースがBAのトレースと等しいという事実から導かれる。このような公式を満たす関数
...
の固有値の重複度を込めた和である。表現の次数がnであれば、その和はn項である。sの位数がmであれば、これらの固有値はすべてm乗の1の冪根である。この事実は
...
を生成することは、すべての既約指標と直交する非零の類関数が存在しないことを示すことで検証できる。表現を
...
この公式は、群の既約表現を同型を除いて分類するという問題に対する「必要十分」条件である。これは、群の既約表現のすべての同型類を見つけたかどうかを確認する手段を我々に提供する。
...
畳み込み代数による表現の記述を用いることで、これらの等式の同値な定式化が得られる。
...
線形表現の性質の節で示したように、群の表現から出発して、制限によって部分群の表現を得ることができる。当然ながら、我々は逆の過程に関心がある。すなわち、部分群の表現から出発して群の表現を得ることは可能であろうか。以下に定義される誘導表現が、必要な概念を提供してくれることがわかるであろう。確かに、この構成は制限の逆ではなく、随伴である。
...
と表記する。表現空間そのものが表現写像の代わりとして頻繁に使用される。すなわち、表現
...
本節で導入された結果は証明なしで提示される。これらは [1]および [2]に見ることができる。
...
ジョージ・マッキーは、誘導表現の既約性を検証するための基準を確立した。これには、まずいくつかの定義と表記に関する仕様が必要となる。
...
本節では、これまでに提示した理論の正規部分群への応用、および部分群とアーベル正規部分群の半直積である特別な群への応用をいくつか紹介する。
...
のアイソタイプ的加群は既約であり、次数は1であり、すべて相似変換であることに注意せよ。
...
の特定の小部分群から構成できることを示すことで分類可能である。
...
のいずれかと同型である。
命題の証明には、とりわけマッキーの基準とフロベニウス相互律に基づく結論が必要である。詳細は[1]を参照されたい。
...
の既約表現である。これは表現環において、次の恒等式として引き継がれる。
...
誘導定理は、与えられた有限群 G の表現環を、G のいくつかの部分群 H からなる族 X の表現環と関連付けるものである。より正確には、そのような部分群の集合に対して、誘導関手は写像を導く。

アルティンの誘導定理は、この種の結果の中で最も初等的な定理である。この定理は、以下の条件が同値であることを主張する。
...
が成り立つ。
Serre (1977)は、この定理の2つの証明を与えている。例えば、Gはその巡回部分群の和集合であるため、
...
のすべての指標は、巡回部分群の指標から誘導された指標の有理数係数の線形結合である。の
...
。巡回群の表現はよく理解されており、特に既約表現は1次元であるため、これはGの表現をある程度制御することを可能にする。
...
が全射であるとは一般には言えない。ブラウアーの誘導定理は、Xがすべての素部分群の族であるならば、
...
のすべての指標は、素部分群の指標によって誘導された指標の整数係数線形結合である。ブラウアーの定理に現れる素部分群Hは、巡回群よりも豊かな表現論を持ち、少なくとも、そのようなHの任意の既約表現は、ある（必然的に同じく素である）部分群
...
の1次元表現によって誘導されるという性質を持つ。（この後者の性質は、任意の超可解群（これには冪零群、特に初等群が含まれる）。このような1次表現から表現を誘導できるという能力は、有限群の表現論においてさらなる帰結をもたらす。
...
値をとる。したがって、実表現の指標は常に実数値であると結論付けることができる。しかし、実数値指標を持つすべての表現が実表現であるとは限らない。これを明確にするために、
...
の任意の行列のトレースは実数であるため、この表現の指標は実数値をとる。
...
が既約でなければ、それはちょうど2つの既約因子を持ち、それらは
...
における共役類（したがって、上記により既約表現）は、の分割に対応する。例えば、
...
3の場合である。このような分割に対して、ヤング図形は分割を視覚的に表現するための図式的な道具である。このような分割（あるいはヤング図形）に対応する既約表現は、スペヒト加群と呼ばれる。
...
の表現を生み出し、その逆も制限によって成り立つ。これらすべての表現環の直和は
...
ではこれは成り立たない。代わりに、カスプ表現として知られる新しい構成要素が必要となる。
...
の表現について記述している。前述のカスプ表現を含む、そのような群の既約表現の幾何学的な記述は、Deligne-Lusztig理論によって得られる。これは、Deligne-Lusztig多様体のl進コホモロジーにおいて、そのような表現を構成するものである。
...
の表現論の類似性は、有限群の枠組みを超えている。カスプ形式の哲学は、これらの群の表現論的側面と、Qpのような局所体上の一般線形群、およびアデール環との親和性を強調している。Bump (2004)を参照のこと。

## 展望—コンパクト群の表現

コンパクト群の表現論は、ある程度まで局所コンパクト群へと拡張できる。この文脈における表現論は、調和解析や保型形式の研究において極めて重要である。証明や詳細な情報、本章の範囲を超えるより深い洞察については、[4]および[5]を参照されたい。

### 定義と性質

位相群とは、群演算と逆元をとる操作が連続であるような位相を備えた群である。そのような群は、
...
の位相における任意の開被覆が有限部分被覆を持つ場合、コンパクトであると呼ばれる。コンパクト群の閉部分群は、再びコンパクトとなる。
...
であるため、最後の条件は省略できる。以下では、特にヒルベルト空間におけるコンパクト群の表現を考察する。
...
有限群の表現のほとんどの性質は、適切な変更を加えることでコンパクト群に引き継ぐことができる。そのために、有限群における和に相当するものが必要となる。
...
はコンパクトであるため、この測度が右不変でもあること、すなわち以下も適用されることを示すことができる。
...
「性質」の節で述べた有限群の表現に関するすべての定義は、コンパクト群の表現にも適用される。ただし、いくつかの修正が必要である。

部分表現を定義するためには、閉部分空間が必要となる。有限次元表現空間の場合はすべての部分空間がすでに閉じているため、これは必要ではなかった。さらに、コンパクト群
...
がユニタリである場合、その2つの表現はユニタリ同値であると呼ばれる。
...
上の二乗可積分関数のヒルベルト空間とする。この空間上の演算子
...
が無限群である場合、これらの表現は有限次元を持たない。冒頭で定義した左正則表現および右正則表現は、群
...
が有限群であるならば、上述の左正則表現および右正則表現と同型である。これは、この場合に
...
与えられた表現から新しい表現を構成する様々な方法は、コンパクト群に対しても同様に用いることができる。ただし、後述する双対表現は例外である。直和およびテンソル積は、有限個の成分や因子を持つ場合、有限群の場合と全く同様に定義される。これは対称積や交代積についても同様である。しかし、コンパクト群の直積に対しては、2つの群の積の既約表現が（同型を除いて）因子群の既約表現のテンソル積に他ならないという定理を拡張するために、ハール測度が必要となる。まず、2つのコンパクト群の直積
...
は、積位相を備えることで再びコンパクト群となることに注意する。直積上のハール測度は、各因子群上のハール測度の積として与えられる。
...
コンパクト群の任意の表現は、既約表現のヒルベルト直和と同型である。
...
同値でない既約表現のアイソタイプは互いに直交することに注意されたい。
...
一般に、コンパクト群の表現はヒルベルト空間やバナッハ空間上で研究される。多くの場合、それらは有限次元ではない。そのため、コンパクト群の表現について語る際に指標に言及することは有用ではない。それにもかかわらず、多くの場合において有限次元のケースに研究を限定することが可能である。

コンパクト群の既約表現は有限次元かつユニタリであるため（最初の小節の結果を参照）、有限群の場合と同様の方法で既約指標を定義できる。

構成された表現が有限次元にとどまる限り、新たに構成された表現の指標は、有限群の場合と同様の方法で得られる。
...
を持つ。
非同型な既約表現が正規直交系であることは既に知られているため、それらが
...
を生成することを確認すればよい。これは、上の非零な二乗可積分関数で、
...
すべての既約指標と直交するものが存在しないことを証明することで達成される。
...
の共役類の数と等しい。しかし、コンパクト群は一般に無限個の共役類を持つため、これは有用な情報をもたらさない。
...
しかし、誘導表現はより一般的に定義可能であり、その定義は部分群
...
フロベニウスの相互律は、内積と双線形形式の修正された定義とともに、コンパクト群へと移行する。この定理は現在、類関数ではなく
...
コンパクト群の表現論におけるもう一つの重要な結果は、ピーター・ワイルの定理である。これは調和解析における中心的かつ基本的な主張の一つであるため、通常は調和解析の文脈で提示され、証明される。
